<template>
  <div id="Sms">
    <van-nav-bar title="短信" :left-text="type === 1 ? '收件箱' : '发件箱'" @click-left="onClickType" @click-right="onSendSms">
      <template #right>
        <van-icon name="plus" size="18" />
      </template>
    </van-nav-bar>
    <SmsList ref="typenum" :key="key" :type="type" @openSendContent="showSendOpen"></SmsList>
    <van-popup v-model:show="show" @closed="closed" round position="bottom" :style="{ height: '80%' }">
      <SendSmsContent @close="closeSendSms" :number="number" :smsContent = "smsContent" ref="close" :key="keys"></SendSmsContent>
    </van-popup>
  </div>
</template>
<script>
...
export default {
  ...
  setup() {
    ...
    // 打开发送窗口并且传参给子组件
    const showSendOpen = (num,content) => {
      number.value = num
